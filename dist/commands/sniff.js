...
function defaultOutPath() {
    ...
    return path.join(appConfigDir(), `sniff-${new Date().toISOString().replace(/[:.]/g, '-')}.ndjson`);
}
...
export async function cmdSniff(opts) {
    ...
    page.on('requestfinished', async (req) => {
        try {
            ...
            if (seen.has(key) && !rurl.includes('graphql'))
                return;
            seen.add(key);
            let postData;
            try {
            ...
            if (res &&
                ...
                try {
                    ...
                    if (bodyText.length > 50_000)
                        bodyText = bodyText.slice(0, 50_000) + '…<truncated>';
                }
                catch {
                    bodyText = undefined;
                }
