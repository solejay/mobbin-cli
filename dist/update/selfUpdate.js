...
function shouldSkip(argv) {
    if (process.env.MOBBIN_NO_AUTO_UPDATE === '1')
        return true;
...
function isCheckDue(state, intervalHours) {
...
function packageJsonPathFromModule() {
    const here = path.dirname(fileURLToPath(import.meta.url));
    return path.resolve(here, '../../package.json');
}
function detectLocalSha(pkgPath) {
    try {
        const raw = readFileSync(pkgPath, 'utf-8');
        const pkg = JSON.parse(raw);
        if (pkg.gitHead)
            return String(pkg.gitHead).trim();
    }
    catch {
        // ignore
    }
...
async function fetchRemoteSha(repo, branch, userAgent) {
...
function spawnBackgroundUpdate(repo, branch) {
...
export async function maybeAutoUpdate(argv) {
    if (shouldSkip(argv))
        return;
    ...
    if (!enabled)
        return;
    const intervalHours = parseIntervalHours(process.env.MOBBIN_AUTO_UPDATE_CHECK_HOURS ?? cfg.autoUpdateCheckHours);
    const state = loadState();
    if (!isCheckDue(state, intervalHours))
        return;
    if (!tryAcquireLock())
        return;
    try {
        ...
        if (localSha)
            state.lastKnownLocalSha = localSha;
        if (remoteSha)
            state.lastKnownRemoteSha = remoteSha;
        saveState(state);
        ...
        if (mode !== 'apply') {
            console.error(`[mobbin] Update available (${shortSha(localSha)} -> ${shortSha(remoteSha)}). ` +
