...
function pickBest(urls) {
    ...
    const preferred = filtered.filter((u) => u.includes('bytescale.mobbin.com') && u.includes('mobbin.com/prod/file.webp') && u.includes('enc='));
    if (preferred.length)
        return preferred[0];
...
export function bestBytescaleUrlFromHtml(html) {
    ...
    const srcSetRe = new RegExp('https:\\\\/\\\\/bytescale\\.mobbin\\.com[^"\\s]+\\s+1920w\\s+https:\\\\/\\\\/bytescale\\.mobbin\\.com[^"\\s]+\\s+3840w', 'g');
...
//# sourceMappingURL=htmlExtract.js.map