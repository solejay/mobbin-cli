...
import type { ScreenAsset } from '../types/models.js';
...
function buildBenchmarkAssets(assets: ScreenAsset[], targetCount: number): ScreenAsset[] {
  if (!assets.length) return [];

  const count = Math.max(1, targetCount);
  const out: ScreenAsset[] = [];
...
export async function cmdBenchmarkDownload(id: string, opts: BenchmarkDownloadOptions) {
  if (!hasStorageState()) {
  ...
  const summaries: Array<{
    ...
    meanDirectSuccess: number;
    meanFallbackSuccess: number;
    meanFailed: number;
    runs: BenchmarkRun[];
  }> = [];

  for (const concurrency of opts.concurrencyList) {
    const runsOut: BenchmarkRun[] = [];
  ...
  for (const summary of summaries) {
    console.log(
      `c=${summary.concurrency} success=${summary.successRuns}/${summary.totalRuns} ` +
    ...
    for (const run of summary.runs) {
      if (run.ok) {
        console.log(
          `  run ${run.run}: ${run.wallMs.toFixed(0)}ms (download=${(run.totalMs ?? 0).toFixed(0)}ms)`,
        );
      } else {
  ...
  if (fastest) {
    console.log('');
    console.log(
      `Recommended concurrency: ${fastest.concurrency} (lowest median wall time ${fastest.medianMs.toFixed(0)}ms)`,
    );
  } else {
    console.log('');
